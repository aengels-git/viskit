...
create_theme<-function(fontsize=18,
                       base_theme=theme_bw,
                       default=T,
                       border_size=NA,
                       axis_size=0.5,
  ...
  if(fontsize<=11){
    ...
    vjust=-0.34308+0.07231*fontsize
  }
  
  if(default){
    base_theme(base_size = fontsize)+
      theme(legend.title = element_text(vjust = vjust,margin = margin(0, 0, 0, 0, "pt")),
            panel.border = element_rect(colour = "black", fill=NA, size=border_size),
            axis.line = element_line(colour = "black",size = axis_size),
            ...)
  }else{
    gg_theme(title_size = fontsize,body_size = fontsize,subtitle_size = fontsize*0.7,caption_size = fontsize*0.7,
             bg_panel_pal = bg_panel_pal,bg_plot_pal = bg_plot_pal,
             ...)+
      theme(legend.title = element_text(vjust = vjust,margin = margin(0, 0, 0, 0, "pt")),
            panel.border = element_rect(colour = "black", fill=NA, size=border_size),
            axis.line = element_line(colour = "black",size = axis_size))
