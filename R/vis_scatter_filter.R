#' @title Shiny App to filter a scatter ggplot by an arbitrary variable before displaying the result.
#'
...
#' @param aggregate Aggregate by col and x value and calculate the mean for each combination (default=FALSE)
#' 
#' @export
#' @examples
  ...
  server <- function(input, output, session) {
    ...
    observeEvent(list(input$filter,input$width,input$height),{
      output$plot<-renderPlot({
        ...
        appData$plot<-do.call("vis_scatter",list_of_arguments[names(list_of_arguments)!=""])
        return(appData$plot)
    ...
    output$downloadPlot <- downloadHandler(
      filename = function(){
        ...
        return(paste0("vis_scatter_plot_",appData$index,'.png'))
        },
...
new_formals<-as.list(formals(vis_scatter))
new_formals<-append(new_formals,list("filter_var"=NULL,"aggregate"=FALSE))
current_enviroment <- rlang::current_env()
formals(vis_scatter_filter, envir = current_enviroment)<-new_formals