#' @title Shiny App to filter a bar ggplot by an arbitrary variable before displaying the result.
#'
...
#' @export
#' @examples
#' vis_barplot_filter(diamonds,x = color,y=prop,text = prop,
#'                    col = cut,y_title = "Percentage",digits = 4,
#'                    y_breaks = scales::breaks_width(0.05),
#'                    y_labels = function(x){scales::percent(x,accuracy = 1)},
#'                    filter_var = carat,position = position_stack(vjust = 0.5))
vis_barplot_filter<-function(...){
  require(viskit)
  require(prepkit)
  ...
  server <- function(input, output, session) {
    ...
    observeEvent(list(input$filter,input$width,input$height),{
      output$plot<-renderPlot({
        ...
        if(aggregate==T){
          data <- prep_freq_table(data,!!!expressions)
        }
        
        list_of_arguments$data<-expr(data)
        list_of_arguments$filter_var<-NULL
        appData$plot<-do.call("vis_barplot",list_of_arguments[names(list_of_arguments)!=""])
        return(appData$plot)
    ...
    output$downloadPlot <- downloadHandler(
      filename = function(){
        ...
        return(paste0("vis_barplot_plot_",appData$index,'.png'))
      },
...
new_formals<-as.list(formals(vis_barplot))
new_formals<-append(new_formals,list("filter_var"=NULL,"aggregate"=TRUE))
current_enviroment <- rlang::current_env()
formals(vis_barplot_filter, envir = current_enviroment)<-new_formals