#' @title Shiny App to filter a ggplot boxplot by an arbitrary variable before displaying the result.
#'
#' @description Create a boxplot plot with a wrapper around the ggplot2::geom_point function.
...
#' @export
#' @examples
#' vis_boxplot_filter(diamonds,x=cut,y=price,col=cut,filter_var=carat)
vis_boxplot_filter<-function(...){
  require(viskit)
  require(prepkit)
  ...
  server <- function(input, output, session) {
    ...
    observeEvent(list(input$filter,input$width,input$height),{
      output$plot<-renderPlot({
        ...
        appData$plot<-do.call("vis_boxplot",list_of_arguments[names(list_of_arguments)!=""])
        return(appData$plot)
    ...
    output$downloadPlot <- downloadHandler(
      filename = function(){
        ...
        return(paste0("vis_boxplot_plot_",appData$index,'.png'))
        },
...
new_formals<-as.list(formals(vis_boxplot))
new_formals<-append(new_formals,list("filter_var"=NULL))
current_enviroment <- rlang::current_env()
formals(vis_boxplot_filter, envir = current_enviroment)<-new_formals